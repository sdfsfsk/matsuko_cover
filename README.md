...
⚠️ **重要提示**：本插件需要配合后端 API 和原版 RVC 或 SVC 使用。

### 1. 下载 RVCSVC-API 后端整合包

- **ModelScope 下载：** https://www.modelscope.cn/models/CCYellowStar/RVCSVC-API/resolve/master/RVCSVC-API.7z
- **GitHub 源码：** https://github.com/CCYellowStar2/RVCSVC-API
...
| 目标 | 操作 |
|------|------|
| 仅使用 RVC | 运行 `启动 rvcapi.bat` |
| 仅使用 SVC | 运行 `启动 svcapi.bat` |
| 两个都要用 | **同时运行两个 bat 文件** |

> ⚠️ 同时也要启动原版的 RVC 或 SVC（SVC 需要替换一个文件，整合包里有说明）

### 3. 模型文件存放

将训练好的 `.pth` 模型文件放入后端对应的模型目录中，然后使用 `/刷新rvc模型` 或 `/刷新svc模型` 命令加载。

详细说明请参考后端整合包中的「使用说明」文档。
...
| 项目 | 最低要求 | 推荐配置 |
|------|---------|---------|
| GPU | NVIDIA 显卡（支持 CUDA） | RTX 3060 及以上 |
| 显存 | 4GB | 8GB 及以上 |
| 内存 | 8GB | 16GB 及以上 |
| Python | 3.9+ | 3.10+ |
| AstrBot | v0.7.0+ | 最新版 |
